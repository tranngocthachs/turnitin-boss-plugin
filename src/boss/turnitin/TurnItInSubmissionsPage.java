...
public class TurnItInSubmissionsPage extends IStaffPluginPage implements
		IStaffAssignmentPluginEntryLink {
	...
	public void handleGet(PageContext pageContext, Template template,
		...
		// Get assignmentId
		String assignmentString = pageContext
				.getParameter(getAssignmentParaName());
		if (assignmentString == null) {
			throw new ServletException("No assignment parameter given");
		}
		Long assignmentId = Long.valueOf(pageContext
				.getParameter(getAssignmentParaName()));
		...
		try {
			...
			// Make sure the corresponding instructor account exists in TurnItIn
			// system
			TIICommResult commResult;
			try {
				commResult = createInstructorAndLogin(staff);
			...
			if (!commResult.isSuccessful()) {
				pageContext.performRedirect(pageContext.getPageUrl(
						StaffPageFactory.SITE_NAME, "tii_error?ec="
								+ commResult.getReturnCode() + "&em="
								+ urlEnc(commResult.getReturnMessage())));
				f.abortTransaction();
				return;
			}
			...
			Collection<StaffSubmissionsQueryResult> submissions = staffInterfaceQueriesDAO
					.performStaffSubmissionsQuery(
							...
							assignmentId);
			Collection<String> reqFilenames = assignmentDao
					.fetchRequiredFilenames(assignmentId);

			// map of submissionId => student
			HashMap<Long, Person> subStdMapping = new HashMap<Long, Person>();

			// map of submissionId => filename of submissions that were already
			// sent to TurnItIn
			HashMap<Long, String> submittedFiles = new HashMap<Long, String>();

			// map of submissionId => reportURLs of submissions that were sent
			// to TurnItIn and reports are available
			HashMap<Long, String> tiiReportURLMapping = new HashMap<Long, String>();

			// map of submissionId => filenames that are available to submit
			// (not yet send to TurnItIn)
			...
			for (StaffSubmissionsQueryResult sub : submissions) {
				...
				Collection<TurnItInSubmission> tiiSubs = tiiSubDao
						.findPersistentEntitiesByExample(tiiSub);
				if (tiiSubs.size() == 1) { // already sent to TurnItIn
					TurnItInSubmission tiiSubmission = tiiSubs.iterator()
							.next();
					String oid = tiiSubmission.getObjectId();
					submittedFiles.put(submission.getId(),
							tiiSubmission.getFilename());
					...
					if (commResult.isSuccessful()
							&& commResult.getReturnCode() == GET_ORI_SCORE_SUCCESS_CODE_FCMD2) {
						// score is available
						tiiReportURLMapping.put(submission.getId(),
								TurnItInComm
										.getOriginalityReportURL(staff, oid));
					} else if (!commResult.isSuccessful()
							&& commResult.getReturnCode() == GET_ORI_SCORE_ERROR_CODE_FCMD2_NOT_YET_AVAI) {
						// score is not yet available, do nothing
					} else {
						// something wrong, redirect to error page
						pageContext
								.performRedirect(pageContext.getPageUrl(
										StaffPageFactory.SITE_NAME,
										"tii_error?ec="
												+ commResult.getReturnCode()
												+ "&em="
												+ urlEnc(commResult
														.getReturnMessage())));
						f.abortTransaction();
					...
					InputStream resIS = f.getResourceDAOInstance()
